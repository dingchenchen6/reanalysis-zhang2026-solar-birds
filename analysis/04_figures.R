...
pA <- ggplot(annL, aes(YEAR, value, colour = variable)) +
  ...
  labs(tag = "A", x = NULL, y = "Change since 2014 (log scale)",
...
pB <- ggplot(sat, aes(x)) +
  ...
  annotate("segment", x = 1, xend = 1, y = 1.46, yend = 1.80, linewidth = 0.3,
           colour = "grey40", arrow = arrow(length = unit(1.4, "mm"))) +
  annotate("text", x = 0.95, y = 1.24,
...
pC <- ggplot(cov, aes(months_observed)) +
  ...
  labs(tag = "C", x = "Months with any birdwatching record (of 120)",
       y = "Counties",
...
fig1 <- (pA | pB) / (pC | pD) + plot_layout(heights = c(1, 1))
