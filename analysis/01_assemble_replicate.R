...
## 政策缺失填零的统计 / policy-imputation counts, reported on two bases:
##   (a) assembled panel — 合并后面板(含仅一次观测的县);
##   (b) estimation sample — 回归样本(fixest 剔除单例固定效应后),
##       正文与评论中引用的即为此口径。
mreg <- m[, if (.N > 1) .SD, by = county]
imp <- data.table(
  basis    = c("assembled panel", "estimation sample (quoted in the Comment)"),
  counties = c(uniqueN(m$id), uniqueN(mreg$id)),
  imputed_counties = c(uniqueN(m[PI_was_imputed == TRUE, id]),
                       uniqueN(mreg[PI_was_imputed == TRUE, id])),
  rows     = c(nrow(m), nrow(mreg)),
  imputed_row_share = c(mean(m$PI_was_imputed), mean(mreg$PI_was_imputed)))
fwrite(imp, file.path(out, "imputation_counts.csv"))
cat("\nPolicy-imputed (PSI set to zero) counties:\n")
print(imp, digits = 3)
