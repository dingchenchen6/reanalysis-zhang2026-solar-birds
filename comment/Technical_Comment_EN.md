...
Zhang *et al*. (*1*) link a photovoltaic policy-stringency index (PSI) to citizen-science bird records aggregated to county-months and conclude that a one-standard-deviation policy increase reduced Shannon diversity by 2.10%. The finding, accompanied by a Perspective (*2*), has been widely reported as evidence that China's solar expansion harms birds. Using the authors' public replication package, we reproduce their preferred estimate exactly (β = −0.0125, clustered SE = 0.0037, *n* = 46,371). The reproduction, however, exposes three problems — concerning measurement, confounding, and inference — each individually sufficient to overturn the conclusion.

**The outcome variable measures the observation process.** The dependent variable is a Shannon index computed from unstructured birdwatching reports within each county-month. In the estimation sample, 38.8% of county-months derive from a single birdwatcher, and the median county is observed in 11 of 120 months (Fig. 1C). Observed richness and Shannon diversity are steep, saturating functions of the number of birdwatchers present (Fig. 1B): county-months with one birder average 15 species (H = 1.90), while those with >100 birders average 142 species (H = 3.60). This is a species-accumulation curve, not a property of bird communities. The authors' own supplementary results carry its signature: PSI associates with *lower* observed richness but *higher* evenness (their table S19) — exactly what shorter species lists produce mechanically, because evenness in our reproduction declines monotonically with effort (0.84 at one birder; 0.73 at >100).

The input data are also unvalidated. The China Ornithological Society reports that 40.5% of the platform's reports are checklist-type records in which every count equals one, which invalidates abundance-weighted indices, and that impossible counts pervade the source data (*3*). At least one such record demonstrably contaminates the published analysis: a single report of 2,147,483,647 little egrets (*Egretta garzetta*; the 32-bit integer maximum) collapses Shannon diversity for Xinhui County in March 2015 to 8 × 10⁻⁷ — in a county whose adjacent months range 2.2–2.9 — and this value enters the regression as an observation of the dependent variable (Fig. 1D). Eighteen further county-months combine ≥20 recorded species with H < 0.2, the same corruption signature. No data-quality screening is described in (*1*).

**Birdwatching effort is confounded with the treatment.** Within the estimation sample, the summed number of birdwatchers grew 171-fold between 2014 and 2023, and observed county-months grew 25-fold, precisely the period over which mean PSI quadrupled (Fig. 1A). The authors control only for `Duration` — the logarithm of birdwatching hours *per birder* — a ratio that discards the explosive growth in total effort. Worse, `Duration` itself responds to PSI (β = −0.0177, *P* < 10⁻⁴), making it a "bad control" that conditions on a post-treatment variable (*4*). PSI strongly predicts effort itself: counties under stricter solar policy record fewer birdwatchers (β = −0.041, *P* = 2 × 10⁻¹¹) and fewer total hours (β = −0.057, *P* = 3 × 10⁻¹³). The estimated "biodiversity" effect is therefore a birdwatching-effort effect wearing an ecological label.

The remedy is standard in the citizen-science literature: model the observation process explicitly (*5*–*8*). Simply adding ln(birdwatchers) to the authors' own specification — same data, fixed effects, and clustering — flips the PSI coefficient to +0.0032 (*P* = 0.31); adding ln(total hours) or both terms gives the same null (Fig. 2A). The richness decline of −0.98 species per PSI unit (their table S19) shrinks by 92% to −0.07 (*P* = 0.37) under the same correction. Splitting the panel confirms the diagnosis: the effect is marginal before the 2020 birdwatching boom (*P* = 0.066), and within 2020–2023 it disappears once effort is controlled (β = +0.0002, *P* = 0.96).

**The estimate is fragile and, at face value, negligible.** The policy variable is equally problematic: 703 of 2,344 sample counties (43.9% of observations) never appear in the policy-text source, and their PSI is imputed as zero even though the source distinguishes true zeros. Restricting the panel to counties actually documented in the policy source yields β = −0.0027 (*P* = 0.49). Even taken at face value, PSI adds 0.0003 to the model's R² — 0.03% of the variance in Shannon diversity, against 31% absorbed by fixed effects (Fig. 2B) — and the implied one-standard-deviation shift (0.052 on H) is 6.5% of the residual standard deviation (0.80). A statistically significant coefficient of this size, in 46,371 observations drawn from a contaminated, effort-driven outcome, carries no ecological information (*9*, *10*).

To establish what the data do support, we crossed every defensible analytical decision — sample (full versus policy-documented counties), cleaning (as published, corrupted rows removed, or ≥2 birders), outcome metric (Shannon, Simpson, log richness), and effort adjustment — into 36 specifications (fig. S1). All seven significant negative estimates come from paths without effort terms; all 18 effort-adjusted paths are null (median +0.10% per standard deviation; range −1.08% to +0.56%). Our preferred estimate — corrupted records removed, policy-documented counties, explicit effort terms — is +0.14% (95% CI, −0.96 to +1.24%) for Shannon diversity, an interval that excludes the published −2.10%.

**Implications.** Our reanalysis does not show that photovoltaic development is harmless; ground-mounted solar can displace habitat, and rigorous impact studies are needed. It shows that this dataset, under this design, cannot detect such an effect in either direction — a distinction with policy consequences. The subgroup results (no significant effect in desert counties) are already being read as support for concentrating gigawatt-scale development in arid lands, yet deserts are precisely where citizen-science coverage is sparsest; absence of evidence is not evidence of absence. Citizen science is transformative for biodiversity monitoring in China, but analyses must model detection and effort explicitly — through occupancy models, list-length corrections, or semi-structured protocols with documented quality control (*5*–*8*, *11*) — and validate extraordinary records before they enter a national-scale dependent variable. We encourage the authors to release the species-level records underlying their indices; an independent econometric audit reaches complementary conclusions (*12*).
...
12. H. Song, Replication and re-analysis of Zhang et al. (2026), *Science* 393, 831–836. figshare (2026). doi:10.6084/m9.figshare.33328569
