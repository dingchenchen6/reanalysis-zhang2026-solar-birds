...
输入数据同样未经验证。在我们项目持有的记录级平台数据中,2014–2023 年 46.9% 的清单不含丰度信息(所有计数均为 1),与中国动物学会鸟类学分会报告的平台全量 40.5% 一致;分会同时在源数据中核实了不可能的计数(*3*)。此类记录无法支持丰度加权的指数。至少一条此类记录可证实污染了已发表的分析:一条 2,147,483,647 只白鹭(*Egretta garzetta*;32 位整数上限)的记录,使广东新会区 2015 年 3 月的香农多样性坍缩至 8 × 10⁻⁷——该县相邻月份的取值范围为 2.2–2.9——而这一数值作为因变量的一个观测**直接进入了回归**(图 1D)。另有 18 个县-月同时呈现"记录物种数 ≥20 而 H < 0.2"的相同污染签名。文(*1*)未描述任何数据质控步骤。
